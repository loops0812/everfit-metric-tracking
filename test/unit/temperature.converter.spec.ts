...
describe('TemperatureConverter', () => {
  describe('toBase (→ °C)', () => {
    ...
    it('should convert 32°F to 0°C (freezing point)', () => {
      expect(converter.toBase(32, TemperatureUnit.FAHRENHEIT)).toBeCloseTo(
        0,
        6,
      );
    });
    ...
    it('should convert 212°F to 100°C (boiling point)', () => {
      expect(converter.toBase(212, TemperatureUnit.FAHRENHEIT)).toBeCloseTo(
        100,
        6,
      );
    });
    ...
    it('should convert 273.15K to 0°C', () => {
      expect(converter.toBase(273.15, TemperatureUnit.KELVIN)).toBeCloseTo(
        0,
        6,
      );
    });
    ...
    it('should convert 0K to -273.15°C (absolute zero)', () => {
      expect(converter.toBase(0, TemperatureUnit.KELVIN)).toBeCloseTo(
        -273.15,
        6,
      );
    });
    ...
    it('should convert 373.15K to 100°C', () => {
      expect(converter.toBase(373.15, TemperatureUnit.KELVIN)).toBeCloseTo(
        100,
        6,
      );
    });
  ...
  describe('fromBase (°C →)', () => {
    it('should convert 0°C to 32°F', () => {
      expect(converter.fromBase(0, TemperatureUnit.FAHRENHEIT)).toBeCloseTo(
        32,
        6,
      );
    });
    ...
    it('should convert 100°C to 212°F', () => {
      expect(converter.fromBase(100, TemperatureUnit.FAHRENHEIT)).toBeCloseTo(
        212,
        6,
      );
    });
    ...
    it('should convert 0°C to 273.15K', () => {
      expect(converter.fromBase(0, TemperatureUnit.KELVIN)).toBeCloseTo(
        273.15,
        6,
      );
    });
    ...
    it('should convert 100°C to 373.15K', () => {
      expect(converter.fromBase(100, TemperatureUnit.KELVIN)).toBeCloseTo(
        373.15,
        6,
      );
    });
  ...
  describe('convert (cross-unit)', () => {
    ...
    it('should round-trip correctly (°C → °F → °C)', () => {
      const original = 36.6;
      const fahrenheit = converter.convert(
        original,
        TemperatureUnit.CELSIUS,
        TemperatureUnit.FAHRENHEIT,
      );
      const backToCelsius = converter.convert(
        fahrenheit,
        TemperatureUnit.FAHRENHEIT,
        TemperatureUnit.CELSIUS,
      );
